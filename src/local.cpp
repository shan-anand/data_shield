...
void local::run_server_sync(const std::string &server_address)
{
  auto common_helper = std::make_shared<CommonHelper>();
  auto block_helper = std::make_shared<ComponentBlockHelper>();

  auto common_service = std::make_shared<CommonServiceImpl>(common_helper);
  auto comp_service = std::make_shared<ComponentBlockServiceImpl>(block_helper);
  ...
  std::cout << "Synchronous server listening on " << server_address << std::endl;
  server->Wait();
}

void local::run_server_async(const std::string &server_address, int poller_threads, int initial_slots)
{
  auto common_helper = std::make_shared<CommonHelper>();
  auto block_helper = std::make_shared<ComponentBlockHelper>();

  AsyncCommonService common_async(common_helper);
  AsyncComponentBlockService comp_async(block_helper);
  ...
  std::unique_ptr<Server> server(builder.BuildAndStart());
  std::cout << "Asynchronous server listening on " << server_address << std::endl;

  common_async.Start(cq.get(), std::max(1, initial_slots / 2));
  comp_async.Start(cq.get(), initial_slots);

  std::vector<std::thread> pollers;
  pollers.reserve(std::max(1, poller_threads));
  for (int i = 0; i < poller_threads; ++i)
  {
